...
async function updateTank(req, res, next){
  ...
  
  {
    ...
    {      
      ...
      // Editing status.
      // =============================================================================
    const minRoleToEditTankStatus = 
          getRoleValueFromName(globals.Settings.minRoleToEditTankStatus) || globals.MaxRoleValue; 
      if (userRoleValue >= minRoleToEditTankStatus){
...
async function getTankDeletePage (req, res, next){
  try {
   ...
   const minRoleValueToDeleteTank = getRoleValueFromName(globals.Settings.minRoleToDeleteTank) || globals.MaxRoleValue; 
...
async function deleteTank (req, res, next){
  try {
    ...
    if (username){
      ...
      const user = await User.get(username);            
      const userRoleValue = getRoleValueFromName(user.role);
    const minRoleValueToDeleteTank = 
          getRoleValueFromName(globals.Settings.minRoleToDeleteTank) || globals.MaxRoleValue; 
      if ((userRoleValue < minRoleToDeleteTank) &&
          (user.id !== tank.submittedBy)) {
        await createServerAudit(user._id, user.username, `Unauthorized user ${user.username} tried to delete the tank ${tank.tankName}.`);
...
async function getRoleEditPage (req, res, next){
  try {
    ...
    if (user.status !== globals.ActiveStatus){
      await createServerAudit(user._id, user.username,  `Inactive user ${user.username} tried to access Role Edit page.`);
...
async function getRoleDeletePage (req, res, next){
  try {
    ...
    if (username){
      ...
      if (user.status !== globals.ActiveStatus){
        await createServerAudit(user._id, user.username,  `Inactive user ${user.username} tried to access Role Delete page.`);
...
async function updateRole(req, res, next){ 
  try {
    ...
    permissionProps.push({ name: 'Eval', key: 'permEval', value: parseInt(req.body.permEval) });
    ...
    if (!isDev && user.status !== globals.ActiveStatus){
      await createServerAudit(user._id, user.username,  `Inactive user ${user.username} tried to update role ${role.name}.`);
    ...
    if (canEditRole){
      ...
      await createServerAudit(user._id, user.username, `User ${user.username} tried to update the role ${role.name}.`);
...
async function createRole(req, res, next){  
  try {
    ...
    if (!isDev && user.status !== globals.ActiveStatus){
      await createServerAudit(user._id, user.username,  `Inactive user ${user.username} tried to create role ${roleProps.name}.`);
    ...
    if (canCreateRole){
      ...
      await createServerAudit(user._id, user.username, `User ${user.username} tried to create the role ${roleProps.name}.`);
...
async function deleteRole(req, res, next){
  try {
    ...
    if (username){
      ...
      if (user.status !== globals.ActiveStatus){
        await createServerAudit(user._id, user.username, `Inactive user ${user.username} tried to delete role ${role.name}.`);
      ...
      if (!canDeleteRole) {
        await createServerAudit(user._id, user.username, `Unauthorized user ${user.username} tried to delete the role ${role.name}.`);
...
async function getSettingsEditPage (req, res, next){
  try {
    ...
    if (username){    
      ...
      if (user.status !== globals.ActiveStatus){
        await createServerAudit(user._id, user.username, `Inactive user ${user.username} tried to access Settings Edit page.`);
...
async function updateSettings (req, res, next){
  try {
    ...
    if (user.status !== globals.ActiveStatus){
      await createServerAudit(user._id, user.username, `Inactive user ${user.username} tried to update tank ${member.username}.`);
...
async function getMemberEditPage (req, res, next){
  try {
    ...
    if (username){
      ...
      if (user.status !== globals.ActiveStatus){
        await createServerAudit(user._id, user.username, `Inactive user ${user.username} tried to access Member Edit page.`);
...
async function getMemberDeletePage (req, res, next){
  try {
    ...
    if (username){
      ...
      if (user.status !== globals.ActiveStatus){
        await createServerAudit(user._id, user.username,  `Inactive user ${user.username} tried to access Member Delete page.`);
...
async function updateMember(req, res, next){ 
  try {
    ...
    if (req.body.status === 'Suspended'){
      ...
      if (req.body.suspendedReason.length <config.minSuspendedReasonLength ){
        return res.status(400).json(createValidationMessage('suspendedReason', `Suspended Reason must be at least ${config.minSuspendedReasonLength} characters long.`));
      }
    }
    // =====================================================================================================            
    ...
    if (username){
      ...
      if (user.status !== globals.ActiveStatus){
        await createServerAudit(user._id, user.username, `Inactive user ${user.username} tried to update member ${member.username}.`);
...
async function deleteMember(req, res, next){
  try {
    ...
    if (username){
      ...
      if (user.status !== globals.ActiveStatus){
        await createServerAudit(user._id, user.username, `Inactive user ${user.username} tried to delete member ${member.username}.`);
      ...
      if (user.id === member.id){
        await createServerAudit(user._id, user.username,  `User ${user.username} tried to delete his/her own account.`);
      ...
      if (userRole.value < minRoleValueToDeleteMember) {
        await createServerAudit(user._id, user.username,  `Unauthorized user ${user.username} tried to delete the member ${member.username}.`);
...
//console.log('[error find test4]')
    ...
    // =====================================================================================
    // Audit trail.
    console.log('[registration]: succesfuly registered a new member');
  ...
  try {
   ...
   if (!user){      
    ...
    } else 
    {
      let msg = null;
          // Last updated by.
      const updatedBy = await User.getById(user.lastUpdatedBy);
      if (updatedBy){
        user.lastUpdatedByUsername = updatedBy.username;
      }
      if (user.status === globals.SuspendedStatus){
        msg = `Account suspended by ${user.lastUpdatedByUsername}`;
      }
...
async function getMapCreatePage (req, res, next){
  ...
  if (user.status !== globals.ActiveStatus){
    await createServerAudit(user._id, user.username,  `Inactive user ${user.username} tried to access Create Map page.`);
...
async function getMapEditPage (req, res, next){
  try {
    ...
    const user = await User.get(username);
    if (user.status !== globals.ActiveStatus){
      await createServerAudit(user._id, user.username,  `Inactive user ${user.username} tried to access Map Edit page.`);
...
async function getMapDeletePage (req, res, next){
  try {
    ...
    if (username){
      ...
      if (user.status !== globals.ActiveStatus){
        await createServerAudit(user._id, user.username,  `Inactive user ${user.username} tried to access Map Delete page.`);
...
async function deleteMap(req, res, next){
  try {
    ...
    if (user.status !== globals.ActiveStatus){
      await createServerAudit(user._id, user.username, `Inactive user ${user.username} tried to delete map ${maze.name}.`);
      return res.status(403).render('unauthorized', { title: 'Unauthorized' });
    }
    ...
    if (!canDeleteMap) {
      await createServerAudit(user._id, user.username,  `Unauthorized user ${user.username} tried to delete the map ${maze.name}.`);
